# [米斯特仰的知识库](README)

_"In the long run we are the makers of our own destiny, in the short run we are the captives of the ideas we have created."_  
_— Friedrich Hayek_

---

## 📚 知识库

- 🤖 **AI技术** — 大语言模型、RAG、Agent、Spring AI 实践
- ☁️ **云原生** — Docker、Kubernetes、MLOps 实战
- 🏛️ **架构设计** — 系统设计、架构思维、技术方法论
- 🇩🇪 **德语学习** — A1 备考、教材笔记、学习计划
- 🇬🇧 **英语学习** — 学习笔记与资源

## 🎯 关于

这是一份个人技术文档库，记录日常工作学习中的笔记与思考。

内容涵盖后端开发（Java、Python）、前端技术（Vue）、云原生架构以及语言学习。

## 📖 最近更新

- 2026/4/5 — [AI时代的云原生学习路径](cloudnative/2026-04-05-ai-era-cloudnative-learning-path)
- 2026/4/5 — [走遍德国第一单元学习笔记](german/2026-04-05-menschen-a1-lektion-1)
- 2026/4/4 — [Spring AI 实践](ai/2026/04-04-spring-ai)
- 2026/4/1 — [AI时代开发者的自我转型](architecture/2026-04-01-ai-era-developer-salvation)

---

_持续更新中..._
